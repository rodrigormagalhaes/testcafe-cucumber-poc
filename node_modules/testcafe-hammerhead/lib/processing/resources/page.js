...
`);
const ORIGIN_FIRST_TITLE_ELEMENT_LOADED_SCRIPT = (0, _createSelfRemovingScript.default)(`
    window["${_internalProperties.default.hammerhead}"].sandbox.node.onOriginFirstTitleElementInHeadLoaded();
`);

const PARSED_BODY_CREATED_EVENT_SCRIPT = _parse.default.parseFragment(BODY_CREATED_EVENT_SCRIPT).childNodes[0];

const PARSED_ORIGIN_FIRST_TITLE_ELEMENT_LOADED_SCRIPT = _parse.default.parseFragment(ORIGIN_FIRST_TITLE_ELEMENT_LOADED_SCRIPT).childNodes[0];
...
class PageProcessor extends _resourceProcessorBase.default {
  ...
  static _addPageOriginFirstTitleParsedScript(head, injectedResources, ctx) {
    ...
    if (nextNode === firstInjectedNode) {
      const taskScriptUrl = ctx.resolveInjectableUrl(_serviceRoutes.default.task);
      const taskScriptNodeIndex = parse5Utils.findNodeIndex(head, node => {
        return node.tagName === 'script' && !!node.attrs.find(attr => attr.name === 'class' && attr.value === _className.default.script) && !!node.attrs.find(attr => attr.name === 'src' && attr.value === taskScriptUrl);
      });
      insertIndex = taskScriptNodeIndex + 1;
    } else insertIndex = firstTitleNodeIndex + 1;
  ...
  processResource(html, ctx, charset, urlReplacer) {
    ...
    const domAdapter = new _parse5DomAdapter.default(processingOpts.isIframe, processingOpts.crossDomainProxyPort.toString());
